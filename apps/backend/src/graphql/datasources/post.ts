...
export default class PostDataSource implements IPostDataSource {
  ...
  async  loadPosts(page: number, limit: number, current_user_id: string): Promise<PostSummary[]> {
  ...
  const appliedPosts = await ApplicationModel.find({ applicant_id: current_user_id, status: 'accepted' }).lean().exec();
  ...
  const postSummaries = posts.map(post => {
    ...
    return {
      ...
      is_applied: appliedPostIds.has(post._id),
...
async  loadPostById(postId: string, current_user_id: string): Promise<PostDetails | null> {
  ...
  const savedPost = await SavedPostModel.findOne({ post_id: postId, user_id: current_user_id }).lean().exec();
  const appliedPost = await ApplicationModel.findOne({ post_id: postId, applicant_id: current_user_id, status: 'accepted' }).lean().exec();

  const isSaved = savedPost !== null;
  const isApplied = appliedPost !== null;
...
async  loadPostByFilter(filter: PostFilterInput, current_user_id: string): Promise<PostSummary[]> {
